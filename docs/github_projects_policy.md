# GitHub Projects (organization workflow)

This repository participates in **zyx-corporation** GitHub **Projects** together with [`kotonoha-management`](https://github.com/zyx-corporation/kotonoha-management), [`kotonoha-spec`](https://github.com/zyx-corporation/kotonoha-spec), [`kotonoha-core`](https://github.com/zyx-corporation/kotonoha-core), [`kotonoha-docs`](https://github.com/zyx-corporation/kotonoha-docs), and **kotonoha-cli (this repository)**.

## For contributors

- Use **GitHub Issues** to propose CLI shape changes and **Pull Requests** to submit implementation or documentation updates.
- **Completion criteria** live in the Issue and PR bodies (and merged commits), not only in a board column.
- Maintainers may mirror progress on shared Organization Projects (status, priority). If an Item is marked **Done** on a board, the linked Issue should reflect the same outcome (typically after the PR is merged).

## Language

Issues and PRs in this repository follow the repository **English-first** policy (see the top-level `README.md`).

## Privacy / public boundary

Do not require private repository names or internal-only codenames in public Issue or documentation text. Keep public artifacts reviewable without access to private repos.

For maintainer-only operational detail (field definitions, triage rules), see internal governance in `kotonoha-management` (private).
