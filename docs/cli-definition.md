...
Implementation notes: **`kotonoha` ≥ 0.1.6** links against **`kotonoha_core`** from [`kotonoha-core`](https://github.com/zyx-corporation/kotonoha-core) (`Cargo.toml` Git dependency on tag **`v0.1.5`**, feature **`postgres`**, for `db` / `interchange store`). Local development MAY override via Cargo **`[patch]`** to a path checkout. RDE validation lives in `kotonoha_core::rde`; interchange envelopes in `kotonoha_core::interchange`; migrations and pool helpers in `kotonoha_core::store::postgres`. See [`docs/spec-traceability.md`](https://github.com/zyx-corporation/kotonoha-core/blob/main/docs/spec-traceability.md).
